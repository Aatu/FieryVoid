<?php
class mentabanaskarancorvette extends MediumShip{
    
    function __construct($id, $userid, $name,  $slot){
        parent::__construct($id, $userid, $name,  $slot);
        
		$this->pointCost = 380;
		$this->faction = "Small Races";
        $this->phpclass = "mentabanaskarancorvette";
        $this->imagePath = "img/ships/mentabanAskaran.png";
        $this->shipClass = "Mentaban Askaran Customs Corvette";
        $this->canvasSize = 100;
		
 		$this->unofficial = true;

		$this->notes = "Uses Raider Mako Breaching Pods.";
		$this->notes .= "<br> ";

		$this->isd = 2253;
        
        $this->forwardDefense = 11;
        $this->sideDefense = 11;
        
        $this->turncost = 0.5;
        $this->turndelaycost = 0.5;
        $this->accelcost = 1;
        $this->rollcost = 1;
        $this->pivotcost = 2;
    	$this->iniativebonus = 60;
		
		$this->fighters = array("assault shuttles"=>6); //Actually Breachin Pods
         
        $this->addPrimarySystem(new Reactor(3, 15, 0, 0));
        $this->addPrimarySystem(new CnC(4, 8, 0, 0));
        $this->addPrimarySystem(new Scanner(3, 12, 4, 5));
        $this->addPrimarySystem(new CargoBay(3, 12));
        $this->addPrimarySystem(new Hangar(3, 6, 2));
    	$this->addPrimarySystem(new Thruster(3, 10, 0, 3, 3));
    	$this->addPrimarySystem(new Thruster(3, 10, 0, 3, 4));
		
        $this->addFrontSystem(new Thruster(3, 6, 0, 2, 1));
        $this->addFrontSystem(new Thruster(3, 6, 0, 2, 1));
        $this->addFrontSystem(new MedBlastCannon(3, 5, 2, 300, 60));
        $this->addFrontSystem(new LtBlastCannon(3, 4, 1, 240, 60));
        $this->addFrontSystem(new LtBlastCannon(3, 4, 1, 300, 120));
        $this->addFrontSystem(new MedBlastCannon(3, 5, 2, 300, 60));		

        $this->addAftSystem(new Engine(3, 11, 0, 6, 2));
        $this->addAftSystem(new LtBlastCannon(3, 4, 1, 180, 360));
        $this->addAftSystem(new LtBlastCannon(3, 4, 1, 0, 180));
        $this->addAftSystem(new Thruster(3, 8, 0, 3, 2));
        $this->addAftSystem(new Thruster(3, 8, 0, 3, 2));
	
        $this->addPrimarySystem(new Structure( 4, 38));
        
        $this->hitChart = array(
        		0=> array(
        				10 => "Thruster",
        				13 => "Cargo Bay",
        				16 => "Scanner",
        				17 => "Hangar",
        				19 => "Reactor",
        				20 => "C&C",
        		),
        		1=> array(
        				6 => "Thruster",
        				8 => "Medium Blast Cannon",
        				10 => "Light Blast Cannon",
        				17 => "Structure",
        				20 => "Primary",
        		),
        		2=> array(
        				6 => "Thruster",
        				8 => "Light Blast Cannon",
        				9 => "Engine",
        				17 => "Structure",
        				20 => "Primary",
        		),
        );
    }

}



?>
