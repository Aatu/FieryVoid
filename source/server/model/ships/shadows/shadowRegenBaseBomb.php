<?php
class shadowRegenBaseBomb extends OSAT{
    ...
    function __construct($id, $userid, $name,  $slot){
        ...
        $this->phpclass = "shadowRegenBaseBomb";
		...
		//Stage S (multi-bay integrated fighters): four ShadowHangars, each served by its
		//OWN Fighter Bomb (added below). bombGroupIndex pairs a bay to its bomb so the bomb
		//launches/drains ONLY that bay's 6 integrated fighters. The structure coupling is
		//carrier-wide (one Structure binds all 24). Each bomb's firing arc matches its
		//hangar's launch sides (a contiguous 240° fan). Side->deg: 0=330..30, 1=30..90,
		//2=90..150, 3=150..210, 4=210..270, 5=270..330.
		$hangar1 = new ShadowHangar(5, 6, 6);
		//$hangar1->directions = array(0, 1, 4, 5);   //sides 4,5,0,1 -> 210..90 fan
		$hangar1->bombGroupIndex = 0;
		$this->addPrimarySystem($hangar1);

		$hangar2 = new ShadowHangar(5, 6, 6);
		//$hangar2->directions = array(0, 1, 2, 5);   //sides 5,0,1,2 -> 270..150 fan
		$hangar2->bombGroupIndex = 1;
		$this->addPrimarySystem($hangar2);

		$hangar3 = new ShadowHangar(5, 6, 6);
		//$hangar3->directions = array(1, 2, 3, 4);   //sides 1,2,3,4 -> 30..270 fan
		$hangar3->bombGroupIndex = 2;
		$this->addPrimarySystem($hangar3);

		$hangar4 = new ShadowHangar(5, 6, 6);
		//$hangar4->directions = array(2, 3, 4, 5);   //sides 2,3,4,5 -> 90..330 fan
		$hangar4->bombGroupIndex = 3;
		$this->addPrimarySystem($hangar4);

		//Stage S (S-f): one Fighter Bomb per ShadowHangar — each launches that bay's pool
		//at a target hex within its arc. ($armour, $maxhealth, $powerReq, $startArc, $endArc,
		//$bombHangarIndex). Arcs mirror each hangar's 240° launch fan; bombHangarIndex pairs
		//to the matching hangar's bombGroupIndex.
		$this->addPrimarySystem(new ShadowFighterBomb(0, 1, 0, 240,  60, 0)); //bay 1 fan
		$this->addPrimarySystem(new ShadowFighterBomb(0, 1, 0, 300, 120, 1)); //bay 2 fan
		$this->addPrimarySystem(new ShadowFighterBomb(0, 1, 0,  60, 240, 2)); //bay 3 fan
		$this->addPrimarySystem(new ShadowFighterBomb(0, 1, 0,  120, 300, 3)); //bay 4 fan
