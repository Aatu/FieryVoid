...
class ThirdspaceCarrier extends BaseShip{
    ...
    function __construct($id, $userid, $name,  $slot){
        ...
        $this->addPrimarySystem(new AdvancedSingularityDrive(8, 40, 0, 44+10+4));
		...
		$this->addPrimarySystem(new ThirdspaceShieldGenerator(6, 20, 0, 40, 3, 4)); //$armor, $maxhealth, $power used, output, maxBoost, boostEfficiency
        $this->addPrimarySystem(new ThirdspaceSelfRepair(7, 16, 8, 3)); //armor, structure, output, maxBoost  
		$this->addPrimarySystem((new JumpEngine(6, 24, 4, 7))->markAncient());//Presumably have access to hyperspace, or possess some other form of FTL travel that this system represents.          		  		
      
	
		$this->addFrontSystem(new ThirdspaceShield(0, 210, 70, 330, 30, 'F'));	
        $this->addFrontSystem(new PsionicConcentrator(5, 0, 0, 240, 120));                        	          	
        $this->addFrontSystem(new GraviticThruster(6, 20, 0, 8, 1));
        $this->addFrontSystem(new GraviticThruster(6, 20, 0, 8, 1));           
			
		
		$this->addAftSystem(new ThirdspaceShield(0, 210, 70, 150, 210, 'A'));		
		...
		$this->addLeftSystem(new ThirdspaceShield(0, 420, 140, 210, 330, 'L'));			
		...
		$this->addRightSystem(new ThirdspaceShield(0, 420, 140, 30, 150, 'R'));
		...
		//structures
        $this->addFrontSystem(new Structure(5, 110));
