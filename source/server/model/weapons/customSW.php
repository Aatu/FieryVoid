...
class SWRayShield extends Shield implements DefensiveSystem
{
    ...
    public function getDefensiveDamageMod($target, $shooter, $pos, $turn, $weapon)
    {
        ...
        //destroyed shield gives no protection
        $output = $this->output + $this->getBoostLevel($turn);
        //Ballistic, Matter, SWIon - passes through!
        if ($weapon->weaponClass == 'Ballistic' || $weapon->weaponClass == 'Matter' || $weapon->weaponClass == 'SWIon') {
            $output = 0;
        }
...

{
    ...
    public function __construct($maxAmount, $startArc, $endArc, $nrOfShots)
    {
        //number of barrels influences FC, too! +1/2 vs fighters, +1/3 vs ships
        if ($this->fireControl[0] !== null) {
            $this->fireControl[0] += floor($nrOfShots / 2);
        }

        if ($this->fireControl[1] !== null) {
            $this->fireControl[1] += floor($nrOfShots / 3);
        }

        if ($this->fireControl[2] !== null) {
            $this->fireControl[2] += floor($nrOfShots / 3);
        }
...

{
    ...
    public function __construct($startArc, $endArc, $noOfLaunchers, $fireControl = null)
    {
    ...
    public function getDamage($fireOrder = null)
    {return $this->damage;}
...
class SWDirectWeapon extends Pulse
{
    ...
    public function __construct($armour, $maxhealth, $powerReq, $startArc, $endArc, $nrOfShots)
    {
        $this->maxpulses = $nrOfShots;
        $this->defaultShots = $nrOfShots;
        //$this->intercept = $nrOfShots; //each weapon needs to calculate this by itself!
    ...
    { //do modify elements not present in usual weapons
        parent::changeFiringMode($newMode);
        if (isset($this->damagebonusArray[$newMode])) {
            $this->damagebonus = $this->damagebonusArray[$newMode];
        }
...
class SWIon extends SWDirectWeapon
{
    ...
    { //make vulnerable to next critical
        ...
        //no damage was actually done
        SWIonHandler::addDamage($ship, $system, $dmg); //possibly cause power shortage
        if ($system->isDestroyed()) {
            return;
        }
        //destroyed system - vulnerability to critical is irrelevant
        if ($system instanceof Structure) {
            return;
        }
        //structure does not suffer critical hits anyway
...
//no point if no damage was actually done
        if ($targetUnit instanceof FighterFlight) {
            return;
        }
//no effect on fighters
        ...
        if (!isset(SWIonHandler::$accumulatedIonDmg[$targetID])) {
            SWIonHandler::$accumulatedIonDmg[$targetID] = 0;
        }
...
class SWFighterLaser extends SWDirectWeapon
{
    ...
    public function __construct($startArc, $endArc, $damagebonus, $nrOfShots)
    {
    ...
    {return Dice::d(6) + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 1 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 6 + $this->damagebonus;}
...
class SWFighterIon extends SWIon
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(4) + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 1 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 4 + $this->damagebonus;}
...

{
    ...
    public function __construct($maxAmount, $startArc, $endArc, $noOfShots)
    {
        //appropriate icon (number of barrels)...
...

{
    ...
    public function __construct($startArc, $endArc, $noOfShots, $fireControl = null)
    {
...
class SWLightLaser extends SWDirectWeapon
{
    ...
    public function __construct($armor, $startArc, $endArc, $nrOfShots)
    { //armor, arc and number of weapon in common housing: structure and power data are calculated!
        $this->intercept = $nrOfShots;
    ...
    public function getDamage($fireOrder)
    {return Dice::d(6) + 2 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 3 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 8 + $this->damagebonus;}
...
class SWMediumLaser extends SWDirectWeapon
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(6) + 4 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 5 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 10 + $this->damagebonus;}
...
class SWHeavyLaser extends SWDirectWeapon
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(8) + 4 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 5 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 12 + $this->damagebonus;}
...
class SWLightTLaser extends SWDirectWeapon
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(6) + 6 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 7 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 12 + $this->damagebonus;}
...
class SWMediumTLaser extends SWDirectWeapon
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(8) + 6 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 7 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 14 + $this->damagebonus;}
...
class SWHeavyTLaser extends SWDirectWeapon
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(6, 2) + 7 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 9 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 19 + $this->damagebonus;}
...
class SWLightLaserE extends SWLightLaser
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(6) + 1 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 2 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 7 + $this->damagebonus;}
...
class SWMediumLaserE extends SWMediumLaser
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(8) + 2 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 3 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 10 + $this->damagebonus;}
...
class SWHeavyLaserE extends SWHeavyLaser
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(8) + 3 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 4 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 11 + $this->damagebonus;}
...
class SWLightTLaserE extends SWLightTLaser
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(6) + 5 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 6 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 11 + $this->damagebonus;}
...
class SWLightIon extends SWIon
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(4) + 4 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 5 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 8 + $this->damagebonus;}
...
class SWMediumIon extends SWIon
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(5) + 5 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 6 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 10 + $this->damagebonus;}
...
class SWHeavyIon extends SWIon
{
    ...
    public function getDamage($fireOrder)
    {return Dice::d(4, 2) + 5 + $this->damagebonus;}
    public function setMinDamage()
    {$this->minDamage = 7 + $this->damagebonus;}
    public function setMaxDamage()
    {$this->maxDamage = 15 + $this->damagebonus;}
...
class SWTractorBeam extends SWDirectWeapon
{
    ...
    protected function onDamagedSystem($ship, $system, $damage, $armour, $gamedata, $fireOrder)
    { //target is held critical on PRIMARY Structure!
        /*
