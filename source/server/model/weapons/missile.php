...
class MissileLauncher extends Weapon
{
    ...
    public function __construct($armour, $maxhealth, $powerReq, $startArc, $endArc, $base = false)
    {
    ...
    { //add testing for ammo explosion!
        return [];
...
class FighterMissileRack extends MissileLauncher
{
    ...
    public function __construct($maxAmount, $startArc, $endArc)
    {
    ...
    public function setSystemDataWindow($turn)
    {
        ...
        if ($distRange > $this->missileArray[$this->firingMode]->range) {
            $this->data["Range"] .= '/' . $this->missileArray[$this->firingMode]->distanceRange;
        }
 ...
 */
    ...
    public function fire($gamedata, $fireOrder)
    { //just decrease ammo and move to standard
        $ammo = $this->missileArray[$fireOrder->firingMode];
        if ($ammo->amount > 0) {
            $ammo->amount--;
