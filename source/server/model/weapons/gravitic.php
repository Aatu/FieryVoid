...
class GraviticMine extends Weapon{
    ...
    /* Moved to HexZone::containsUnit() - the Walkers' Energy Draining Net needs the same
       zone test (pairwise corridors and closed-area fill), and the geometry was never
       about mines. The convex hull, point-in-polygon, per-angle touch tolerance and
       point-to-segment helpers went with it, unchanged. See WALKERS_OF_SIGMA_PLAN.md 2.3. */
    private function isUnitInShearingZone(OffsetCoordinate $unitPos, array $minePositions){
        return HexZone::containsUnit($unitPos, $minePositions);
    }
