...
class ThirdspaceShieldGenerator extends ShipSystem{
	...
	public function criticalPhaseEffects($ship, $gamedata){
		...
		foreach ($allShields as $shield) {							
			...
			if($currentShieldHealth >= $totalShieldRating) continue;//Don't regen shields if total shield power for ship is equal (or more) than max allowed.
		...
		while ($spareEnergy > 0) { //
			...
			if($currentShieldHealth >= $totalShieldRating) break;//Don't regen shields if total shield power for ship is equal (or more) than max allowed.
