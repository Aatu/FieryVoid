...
class ThirdspaceShieldGenerator extends ShipSystem{
    ...
    public function setSystemDataWindow($turn){
        ...
        $this->data["Special"] .= "<br>Shield's cannot regenerate more than their Normal Strength, however if they are full the Generator will try and allocate their excess energy to another shield.";
	...
	public function criticalPhaseEffects($ship, $gamedata){
		...
		foreach ($allShields as $shield) {							
			...
			if($currentShieldHealth >= $totalShieldRating) continue;//Don't regen a shields if total shield power for ship is equal (or more) than max allowed.
		...
		if($currentShieldHealth >= $totalShieldRating) return; //If for some reason total shield health is equal/greater than baseRatings combined, don't give out spare energy!					
