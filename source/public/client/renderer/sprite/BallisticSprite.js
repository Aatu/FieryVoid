...
window.BallisticSprite = function () {
    ...
    function BallisticSprite(position, type, text = "", textColour = "#ffffff", imageSrc = null, opacity = 1) {
        ...
        if (imageSrc) {
            ...
            if (!TEXTURE_HEX_ORANGE) {
                createTextures(opacity); // Initialize all textures once
            }
    ...
    function createTexture(type, opacity = 1) {

        var strokeOpacity = Math.round((0.5 / opacity) * 100) / 100;
        var fillOpacity   = Math.round((0.1 / opacity) * 100) / 100;       
        const stroke = getStrokeColorByType(type, strokeOpacity);
        const fill   = getFillColorByType(type, fillOpacity);

        var canvas = HexagonTexture.renderHexGrid(TEXTURE_SIZE, stroke, fill, 10);
...
function getStrokeColorByType(type, opacity = 0.5) {
    switch (type) {
        ...
        case "hexWhite":  return `rgba(255,255,255,${opacity*0.8})`;
        case "hexClear":  return "rgba(0,0,0,0)";
        default:          return `rgba(144,185,208,${opacity})`;
    }
}

function getFillColorByType(type, opacity = 0.1) {
    switch (type) {
        case "hexOrange": return `rgba(250,110,5,${opacity})`;
        case "hexRed":    return `rgba(230,20,10,${opacity})`;
        case "hexBlue":   return `rgba(0,184,230,${opacity})`;
        ...
        case "hexYellow": return `rgba(255,255,0,${opacity})`;
        case "hexPurple": return `rgba(127,0,255,${opacity})`;
        case "hexWhite":  return `rgba(255,255,255,${opacity})`;
        case "hexClear":  return "rgba(0,0,0,0)";
        default:          return `rgba(144,185,208,${opacity*3})`;
    }
}
