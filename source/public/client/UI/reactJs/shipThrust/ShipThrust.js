...
const ThrusterSetContainer = styled.div`
  position: relative;
  transform: ${props => `rotate(${props.rotation}deg);`} ${Text} {
      ...
      >
        ...
        <ThrustTooltip>
          ...
          <TooltipEntryButton
            space
            important
            onClick={this.resetThrust.bind(this)}
          >
            RESET THRUST
          </TooltipEntryButton>
          <TooltipEntryButton important onClick={this.autoAssign.bind(this)}>
            AUTO ASSIGN
          </TooltipEntryButton>
          <ThrustButtons>
...
const getTurnDelay = (ship, movement) => {
  ...
  const turndelay = shipManager.movement.calculateTurndelay(
    ship,
    movement,
    movement.speed
  );
...
const getText = (totalRequired, remainginRequired, movement) => {
  ...
  const list = remainginRequired
    .map((required, index) => {
      ...
      return (
        <TooltipEntry
          type={required === 0 ? "good" : "bad"}
          key={`assign-thrust-text-${index}`}
        >
          {required} thrust to {names[index]} thrusters
        </TooltipEntry>
      );
    })
...
const getThrusters = (ship, direction, totalRequired, movement) => {
  ...
  return thrusters.map((thruster, index) => {
    ...
    let crits = shipManager.criticals.hasCritical(thruster, "HalfEfficiency")
      ? 10
      : 0;
    ...
    return (
      <Thruster
        crits={crits}
        onClick={assignThrust}
        onContextMenu={unAssignThrust}
        direction={direction}
