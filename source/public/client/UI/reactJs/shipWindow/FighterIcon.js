  ...
  render() {
    ...
    return (
      <FighterIconContainer
        destroyed={destroyed}
        img={fighter.iconPath}
        onMouseOver={this.onSystemMouseOver.bind(this)}
        onMouseOut={this.onSystemMouseOut}
      >
        <Container>
        ...
        <HealthBar
          health={getStructureLeft(ship, fighter)}
...
const getNotWeapons = fighter =>
  fighter.systems.filter(system => !system.weapon);
  ...
  return systems.map((system, i) => {
    return (
      <SystemIcon
        ...
        fighter={true}
        scs
        key={`system-scs-fighter${fighter.id}-${ship.id}-${system.id}-${i}`}
        system={system}
        ship={ship}
      />
    );
