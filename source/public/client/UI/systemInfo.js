...
window.systemInfo = {
  showSystemInfo: function showSystemInfo(element, system, ship, selectedShip) {
    ...
    if (!ship.flight) {
      h +=
        '<div><span class="header">Structure:</span><span class="value">' +
        (system.maxhealth - damageManager.getDamage(ship, system)) +
        "/" +
        system.maxhealth +
        "</span></div>";
      h +=
        '<div><span class="header">Armor:</span><span class="value">' +
        shipManager.systems.getArmour(ship, system) +
        "</span></div>";
    } else {
      h +=
        '<div><span class="header">Offensive bonus:</span><span class="value">' +
    ...
    if (system.firingModes)
      h +=
        '<div><span class="header">Firing mode:</span><span class="value">' +
        system.firingModes[system.firingMode] +
        "</span></div>";

    if (!mathlib.arrayIsEmpty(system.missileArray)) {
      h +=
        '<div><span class="header">Ammo Amount:</span><span class="value">' +
    ...
    if (
      ...
      if (weaponManager.canCalledshot(ship, system, selectedShip)) {
        var e = $(
          '<div class="calledtargeting"><span>CALLED SHOT</span></div><div class="targeting"></div>'
        );
