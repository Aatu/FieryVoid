...
window.shipClickable = {
	...
	shipclickableMouseOver: function shipclickableMouseOver(e) {
		...
		if ($(this).hasClass('shiplistentry')) shipClickable.testStacked = false;else shipClickable.testStacked = true;
	},

	shipclickableMouseOut: function shipclickableMouseOut(e) {
		clearTimeout(shipClickable.shipClickableTimer);
		...
		if (ship) shipManager.drawShip(ship);
	},

	doMouseOver: function doMouseOver() {
		...
		if (shipSelectList.haveToShowList(ship) && shipClickable.testStacked) {
			...
			shipClickable.addEntryElement("Defence (F/S): " + fDef + "(" + ship.forwardDefense * 5 + ") / " + sDef + "(" + ship.sideDefense * 5 + ")%");
		...
		if (shipSelectList.haveToShowList(ship) && shipClickable.testStacked) {
			...
			if (gamedata.isEnemy(ship) && (gamedata.gamephase == 3 && shipManager.systems.selectedShipHasSelectedWeapons() || gamedata.gamephase == 1 && shipManager.systems.selectedShipHasSelectedWeapons(true)) && gamedata.waiting == false) {
	...
	addEntryElement: function addEntryElement(value, condition) {
		if (condition === false || condition === 0) return;

		var s = value;
		if (condition !== true && condition) s += condition;

		$('<div class="entry"><span class="value">' + s + '</span></div>').insertAfter('#shipNameContainer .namecontainer');;
	}
