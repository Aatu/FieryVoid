...
window.gamedata = {
    ...
    if (gamedata.gamephase == 1) {
          ...
          for (var syst in myShips[ship].systems) {
            if (myShips[ship].systems[syst].name == "reactor") {
              for (var pow in myShips[ship].systems[syst].power) {
                if (
                  myShips[ship].systems[syst].power[pow].turn ==
                    gamedata.turn &&
      ...
      if (selfDestructing.length > 0) {
        ...
        for (var ship in selfDestructing) {
          html +=
            selfDestructing[ship].name +
            " (" +
            selfDestructing[ship].shipClass +
            ")";
          html += "<br>";
        }
        html += "<br>";
        ...
        if (!myShips[ship].flight) {
          ...
          for (var i = 0; i < myShips[ship].systems.length; i++) {
            if (typeof myShips[ship].systems[i] != "undefined") {
              for (
                var j = 0;
                j < myShips[ship].systems[i].systems.length;
                j++
              ) {
                if (typeof myShips[ship].systems[i].systems[j] != "undefined") {
                  if (
                    myShips[ship].systems[i].systems[j].fireOrders.length > 0
                  ) {
                    fired = 1;
                    break;
                  }
  ...
  doCommit: function doCommit() {
    ...
    if (gamedata.gamephase == 1) {
      ...
      if (shipNames.length > 0) {
        ...
        negPowerError +=
          "You need to turn off systems before you can commit the turn.";
      ...
      if (shipNames.length > 0) {
        ...
        tooManyShieldsError +=
          "You need to turn off shields or boost your shield generator before you can commit the turn.";
  ...
  drawIniGUI: function drawIniGUI() {
    ...
    for (var i = 0; i < ships.length; i++) {
      ...
      var categoryIndex = window.SimultaneousMovementRule.getShipCategoryIndex(
        ships[i]
      );
      ...
      span.innerHTML +=
        "<p style='margin-top: 6px; margin-bottom: 6px; font-size: 12px'>" +
        ships[i].name;
      span.innerHTML +=
        "<p style='margin-top: 6px; margin-bottom: 6px; font-style: italic; font-weight: bold'>" +
        ships[i].shipClass;
      ...
      if (active !== null) {
        ...
        if (gamedata.getActiveShips().includes(ships[i])) {
          td.className = gamedata.isMyShip(ships[i])
            ? "iniActive"
            : "iniActiveEnemy";
        }
      }
  ...
  showSurrenderButton: function showSurrenderButton() {
    $(".surrender")
      .on("click", gamedata.onSurrenderClicked)
      .show();
  },

  hideSurrenderButton: function hideSurrenderButton() {
    $(".surrender")
      .off("click", gamedata.onSurrenderClicked)
      .hide();
  },
