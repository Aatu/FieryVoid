...
window.gamedata = {
    ...
    edfHexes: null,
    /* Walkers of Sigma-957 (WALKERS_OF_SIGMA_PLAN.md 3.7, Stage 7) - the hexes generated by
       Energy Draining NETS (their own hexes, the corridors between linked Nets and any filled-in
       closed area), as a flat [{q, r}]. NULL when no Net is on the board.
       ⚠️ A RENDERING HINT, NOT AN AUTHORITY. Every one of these hexes is also in edfHexes above,
       team-tagged, and that is what the to-hit penalty reads. This list exists only because a
    ...
    /* Walkers of Sigma-957 (Stage 7) - the SAME list, recomputed locally from PLOTTED positions so
       ...
       BallisticIconContainer.generateEdfNetHexes, and NULL outside deployment and movement.
       ⚠️⚠️ NEVER COPIED FROM THE SERVER, and never read by a rule. It is the output of a client
       mirror (model/EdfNetLinks.js) and exists purely to draw with; every rule - the to-hit
       penalty, the drain, own-fleet immunity - reads edfHexes, which is the server's alone. */
    edfNetPreview: null,
...
*/
    ...
    parseServerData: function parseServerData(serverdata) {
        ...
        /* Walkers of Sigma-957 (Stage 7) - the hexes an Energy Draining NET generates, for the
           map overlay. Same named-key rule, same null normalisation. The failure if it is
           forgotten is quieter than edfHexes' (the penalty still works, because that reads
