...
shipManager.systems = {
  selectedShipHasSelectedWeapons: function selectedShipHasSelectedWeapons(
    ship,
    ballistic
  ) {
  ...
  getArmour: function getArmour(ship, system) {
    var armour =
      system.armour - shipManager.criticals.hasCritical(system, "ArmorReduced");
  ...
  isReactorDestroyed: function isReactorDestroyed(ship) {
    ...
    if (ship.base) {
      return shipManager.systems.isDestroyed(
        ship,
        shipManager.systems.getSystemByNameInLoc(ship, "reactor", 0)
      );
    }
  ...
  getOutput: function(ship, system) {
    ...
    var output =
      system.output + system.outputMod + shipManager.power.getBoost(system);
  ...
  getArcs: function getArcs(ship, weapon) {
    if (shipManager.movement.isRolled(ship)) {
      return {
        start: mathlib.addToDirection(weapon.endArc, weapon.endArc * -2),
        end: mathlib.addToDirection(weapon.startArc, weapon.startArc * -2)
      };
    } else {
  ...
  getSystemsForShipStatus: function getSystemsForShipStatus(ship, location) {
    ...
    for (var i in ship.systems) {
      if (
        ship.systems[i].location == location &&
        ship.systems[i].name != "structure"
      )
    ...
    for (var i in systems) {
      ...
      for (var a in grouped) {
        for (var b in grouped[a]) {
          if (
            !found &&
            (grouped[a][b].name == system.name ||
              (grouped[a][b].primary && system.primary)) &&
            grouped[a].length < 4
          ) {
