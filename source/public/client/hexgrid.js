...
window.hexgrid = {
    ...
    hexHeight: function hexHeight() {
        return hexgrid.hexlenght * gamedata.zoom * 1.5;
    },
    hexWidth: function hexWidth() {
        return hexgrid.hexlenght * gamedata.zoom * 0.8660254 * 2;
    },
    selectedHex: null,

    drawHexGrid: function drawHexGrid() {

        var renderer = new window.canvasHexGridRenderer(hexgrid, gamedata, graphics, deployment);
        renderer.renderHexGridOnCanvas("hexgrid");
    },

    drawGameSpace: function drawGameSpace(pos) {
    ...
    positionToPixel: function positionToPixel(pos) {
        if (!pos) return null;

        var pixpos = hexgrid.hexCoToPixel(pos.x, pos.y);

        if (pos.xO) pixpos.x = pixpos.x + pos.xO * gamedata.zoom;

        if (pos.yO) pixpos.y = pixpos.y + pos.yO * gamedata.zoom;

        return pixpos;
    },

    //ATTENTION, this is bloody magic! (I don't really know how it works)
    pixelCoToHex: function pixelCoToHex(px, py) {
    ...
    hexCoToPixel: function hexCoToPixel(hx, hy) {
        origoHexX = gamedata.scroll.x;
        origoHexY = gamedata.scroll.y;
    ...
    getHexToDirection: function getHexToDirection(d, x, y) {
        var pos = { x: x, y: y };
    ...
    isOccupiedPos: function isOccupiedPos(pos) {
        var others = Array();
    ...
    getOffsetPositionInHex: function getOffsetPositionInHex(pos, direction, percentage, unoccupied) {
