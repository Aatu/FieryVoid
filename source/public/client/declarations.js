...
window.declarations = {
  ...
  readDeclarationsEW: function readDeclarationsEW(){
    ...
    for (var i in gamedata.ships){
      ...
      if ( (!shipManager.isDestroyed(ship)) || (shipManager.turnDestroyed(ship)>=gamedata.turn) ) if( (declarations.GlobalSide=='Own' && declarations.GlobalDisplay=='Source' && gamedata.isMyShip(ship)) //own ship, own ew, by source
  ...
  readDeclarationsFire: function readDeclarationsEW(){
    ...
    for (var i in gamedata.ships){
      var ship = gamedata.ships[i];
      if ( (!shipManager.isDestroyed(ship)) || (shipManager.turnDestroyed(ship)>=gamedata.turn) ) if( (declarations.GlobalSide=='Own' && declarations.GlobalDisplay=='Source' && gamedata.isMyShip(ship)) //own ship, own fire, by source
