...
window.weaponManager = {
  ...
  getWeaponCurrentLoading: function getWeaponCurrentLoading(weapon) {
    if (weapon.duoWeapon) {
      var returnArray = new Array(
        weapon.weapons[1].getTurnsloaded(),
        weapon.weapons[2].getTurnsloaded()
      );
      return returnArray;
    }
  ...
  onWeaponMouseover: function onWeaponMouseover(e) {
    ...
    if ($(this).hasClass("fightersystem")) {
      weaponManager.currentSystem = shipManager.systems.getFighterSystem(
  ...
  doWeaponMouseOver: function doWeaponMouseOver(e) {
    ...
    var weapon = weaponManager.currentSystem;
  ...
  checkConflictingFireOrder: function checkConflictingFireOrder(
    ship,
    weapon,
    alert
  ) {
    ...
    for (var i in p.systems) {
      ...
      if (weaponManager.hasFiringOrder(ship, system)) {
        ...
        if (
          (weapon.exclusive || system.exclusive) &&
          weapon.ballistic == system.ballistic
        ) {
          if (alert)
            confirm.error(
              "You cannot fire <b>" +
  ...
  selectWeapon: function selectWeapon(ship, weapon) {
    ...
    if (!weaponManager.isLoaded(weapon)) return;
        ...
        for (var b = 0; i < ship.systems.systems; b++) {
          if (ship.systems[i].systems[b].weapon) {
            ...
            shipWindowManager.setDataForSystem(
              ship,
              ship.systems[i].systems[b].weapon
            );
          }
        }
      }
  ...
  selectAllWeapons: function selectAllWeapons(ship, system) {
    ...
    array = systems.filter(function(weapon) {
      return weapon.displayName === system.displayName;
    });
  ...
  targetingShipTooltip: function targetingShipTooltip(
    selectedShip,
    ship,
    e,
    calledid
      ...
      if (weaponManager.isOnWeaponArc(selectedShip, ship, weapon)) {
        if (weaponManager.checkIsInRange(selectedShip, ship, weapon)) {
          ...
          if (calledid != null && !weaponManager.canWeaponCall(weapon)) {
            ...
            $(
              '<div><span class="weapon">' +
                weapon.displayName +
                ':</span><span class="hitchange"> CANNOT CALL SHOT</span></div>'
            ).appendTo(f);
          } else {
            $(
              '<div><span class="weapon">' +
                weapon.displayName +
                ':</span><span class="hitchange"> - Approx: ' +
          ...
          $(
            '<div><span class="weapon">' +
              weapon.displayName +
              ':</span><span class="hitchange"> NOT IN RANGE</span></div>'
          ).appendTo(f);
        }
      } else {
        $(
  ...
  canWeaponCall: function canWeaponCall(weapon) {
    ...
    if (weapon.damageType == "Standard" || weapon.damageType == "Pulse")
      return true;
    return false;
  },
  ...
  canCalledshot: function canCalledshot(target, system, shooter) {
    ...
    if (
      mathlib.isInArc(
        shooterCompassHeading,
        mathlib.addToDirection(system.startArc, targetFacing),
        mathlib.addToDirection(system.endArc, targetFacing)
      )
    ) {
      return true;
    }
  ...
  canCalledshotOld: function canCalledshotOld(target, system) {
    ...
    if (target.shipSizeClass == 3) {
      ...
      /*target.shipSizeClass == 1 ||*/ target.shipSizeClass == 2 &&
      system.name == "thruster" &&
      system.location == 0
    ) {
      var thruster = weaponManager.getTargetableThruster(shooter, target);
  ...
  isPosOnWeaponArc: function isPosOnWeaponArc(shooter, position, weapon) {
    ...
    var targetCompassHeading = mathlib.getCompassHeadingOfPoint(
  ...
  calculataBallisticHitChange: function calculataBallisticHitChange(
    ball,
    calledid
  ) {
    ...
    if (shooter.flight) {
      return weaponManager.calculateFighterBallisticHitChange(
        shooter,
        target,
        weapon,
        calledid
    ...
    var baseDef = weaponManager.calculateBaseHitChange(
      target,
      defence,
      shooter,
      weapon
    ...
    if (!shooter.flight && !shooter.osat)
      mod -= shipManager.criticals.hasCritical(
        shipManager.systems.getSystemByName(shooter, "cnC"),
        "PenaltyToHit"
      );
    ...
    var goal =
      baseDef - rangePenalty - intercept + oew + soew + firecontrol + mod;
  ...
  calculateBaseHitChange: function calculateBaseHitChange(
    target,
    base,
    shooter,
    weapon
  ...
  calculateRamChance: function(shooter, target, weapon, calledid) {
    ...
    switch (targetSpeed) {
      ...
      case 5:
        ...
        break;
      default:
        //this means >5; ‐1 for every 5 points of speed (or fraction thereof) that the target is moving faster than 5.
  ...
  calculateHitChange: function calculateHitChange(
    shooter,
    target,
    weapon,
    calledid
    ...
    if (shooter.flight) {
      ...
      var OBcrit = shipManager.criticals.hasCritical(
        firstFighter,
        "tmpsensordown"
      );
    ...
    if (oew < 1 && !shooter.flight) {
      ...
      if (jammer && !shipManager.power.isOffline(target, jammer))
        jammermod =
          rangePenalty * shipManager.systems.getOutput(target, jammer);
    ...
    var goal =
      baseDef - jammermod - rangePenalty + oew + soew + firecontrol + mod;
  ...
  calculateFighterBallisticHitChange: function calculateFighterBallisticHitChange(
    shooter,
    target,
    weapon,
    calledid
    ...
    if (
      ...
      weaponManager.isPosOnWeaponArc(shooter, tPosHex, weapon)
    ) {
      oew = shooter.offensivebonus;
    }
  ...
  getShipDefenceValuePos: function getShipDefenceValuePos(position, target) {
    ...
    var shooterCompassHeading = mathlib.getCompassHeadingOfPoint(
      targetPos,
      position
    );
  ...
  targetBallistic: function targetBallistic(ship, ball) {
    ...
    for (var i in gamedata.selectedSystems) {
      ...
      if (
        shipManager.systems.isDestroyed(selectedShip, weapon) ||
        !weaponManager.isLoaded(weapon)
      )
        continue;
  ...
  confirmSelfIntercept: function confirmSelfIntercept(
    ship,
    valid,
    invalid,
    message
  ...
  setSelfIntercept: function setSelfIntercept(ship, valid) {
    for (var weapon in valid) {
      ...
      var fireid =
        ship.id + "_" + weapon.id + "_" + (weapon.fireOrders.length + 1);
  ...
  targetShip: function targetShip(selectedShip, ship, system) {
    ...
    for (var i in gamedata.selectedSystems) {
      ...
      if (weapon.ballistic && gamedata.gamephase != 1) {
        debug &&
          console.log("trying to fire in wrong phase for ballistic weapon");
        continue;
      }
      ...
      if (
        !ship.flight &&
        ship.shipSizeClass < 2 &&
        weapon.fireControl[1] === null
      ) {
      ...
      if (weaponManager.isOnWeaponArc(selectedShip, ship, weapon)) {
        ...
        if (weaponManager.checkIsInRange(selectedShip, ship, weapon)) {
          ...
          for (var s = 0; s < weapon.guns; s++) {
            ...
            var damageClass = weapon.data["Weapon type"].toLowerCase();
            var chance = weaponManager.calculateHitChange(
              selectedShip,
              ship,
              weapon,
              calledid
  ...
  checkIsInRange: function checkIsInRange(shooter, target, weapon) {
    ...
    if (jammer) {
      ...
      if (
        !shipManager.power.isOfflineOnTurn(target, jammer, gamedata.turn - 1)
      ) {
  ...
  targetHex: function targetHex(selectedShip, hexpos) {
    ...
    for (var i in gamedata.selectedSystems) {
      ...
      if (weaponManager.isPosOnWeaponArc(selectedShip, hexpos, weapon)) {
        if (
          weapon.range === 0 ||
          shipManager.getShipPosition(selectedShip).distanceTo(hexpos) <=
            weapon.range
        ) {
          weaponManager.removeFiringOrder(selectedShip, weapon);
          for (var s = 0; s < weapon.guns; s++) {
      ...
      if (
        ...
        if (
          ((gamedata.gamephase == 1 || gamedata.gamephase == 3) &&
            system.ballistic) ||
          (gamedata.gamephase == 3 && !system.ballistic)
        ) {
  ...
  getAllFireOrdersForDisplayingAgainst: function getAllFireOrdersForDisplayingAgainst(
    target
  ) {
    return gamedata.ships
      .reduce(function(fires, shooter) {
      ...
      for (var i in system.weapons) {
        ...
        if (weapon.duoWeapon) {
          for (var index in weapon.weapons) {
            ...
            fires = fires.concat(
              weaponManager.getAllFireOrdersFromSystem(subweapon)
            );
          }
        } else {
          fires = fires.concat(
            weaponManager.getAllFireOrdersFromSystem(weapon)
          );
        }
      }
    }
  ...
  isLoaded: function isLoaded(weapon) {
    return (
      weapon.loadingtime <= weapon.turnsloaded ||
  ...
  getAllFireOrdersForAllShipsForTurn: function getAllFireOrdersForAllShipsForTurn(
    turn,
    type
  ) {
    var fires = [];
